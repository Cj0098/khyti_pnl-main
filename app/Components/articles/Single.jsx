...
import FetchSingleArticle from "../../services/fetchSingleArticle";
const SingleArticle = ({ params }) => {
  ...
  const handleDeleteGal = (gal) => {
    setGallery((prevGallery) => prevGallery.filter((item) => item !== gal));
  };
  ...
  useEffect(() => {
    ...
    FetchSingleArticle(token, params.id).then((res) => {
      setArticle(res.data);
      setTitle(res.data.name);
      setBodyContent(res.data.content);
      setIsPublished([true, res.data.id]);
      setThumbnail(res.data.img);
      setCat(res.data.cat_id);
      SetPublishDate(res.data.release);
      // setGallery(res.data.gallery);
      let gal = [];
      res.data.gallery.map((g) => {
        gal.push(g.img);
      });
      setGallery(gal);
      let tg = [];
      ...
      setTagsArray(tg);
      // setTagsArray(Object.values(res.data.tag));
